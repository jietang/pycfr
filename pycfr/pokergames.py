...
def half_street_kuhn_rules(infoset_format=leduc_format):
    players = 2
    deck = [Card(14,1),Card(13,1),Card(12,1)]
    ante = 1
    blinds = None
    rounds = [RoundInfo(holecards=1,boardcards=0,betsize=1,maxbets=[1,0])]
    return GameRules(players, deck, rounds, ante, blinds, handeval=kuhn_eval, infoset_format=infoset_format)
...
def kuhn_rules(infoset_format=leduc_format):
    players = 2
    deck = [Card(14,1),Card(13,1),Card(12,1)]
    ante = 1
    blinds = None
...
def leduc_rules(infoset_format=leduc_format):
    players = 2
    ...
    return GameRules(players, deck, rounds, ante, blinds, handeval=leduc_eval, infoset_format=infoset_format)
...
def royal_rules(infoset_format=royal_format):
    players = 2
    ...
    return GameRules(players, deck, rounds, ante, blinds, handeval=royal_eval, infoset_format=infoset_format)
